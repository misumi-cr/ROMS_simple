...
!
!svn $Id: inp_par.F 662 2013-05-03 15:39:23Z arango $
!================================================== Hernan G. Arango ===
...
#endif
      integer :: inp, out, i, ic, ifield, itrc, ivar, ng, npts
      integer :: sequence
...
!-----------------------------------------------------------------------
!
      OPEN (16, FILE=TRIM(cparnam), FORM='formatted', STATUS='old')

      CALL read_CoagPar (model, 16, out, Lwrite)
#endif
#ifdef SEDIMENT
!
!-----------------------------------------------------------------------
...
!
 ...
 120    FORMAT (/,1x,'Lateral Boundary Conditions: ',a,/,1x,28('='),/,  &
     &          /,1x,'Variable',t25,'Grid',t33,'West Edge',             &
     &          t45,'South Edge', t57,'East Edge',t69,'North Edge',     &
     &          /,1x,'---------',t25,'----',t33,4('----------',2x))
        DO ifield=1,nLBCvar
...
#if defined TINFLUX
!
!  Determine the number of 3D tracer influx to process.
!
        ic=0
        DO itrc=1,NT(ng)
          IF (LtracerFLX(itrc,ng)) THEN
            ic=ic+1
          END IF
        END DO
        NTFLX(ng)=ic
      ...
      IF ((0.lt.ifield).and.(ifield.le.nLBCvar)) THEN
        DO ibry=1,4
          ...
          SELECT CASE (TRIM(string))
            CASE ('CHA')
              S(ibry,ifield,igrid)%Chapman   = .TRUE.
            CASE ('CLA')
